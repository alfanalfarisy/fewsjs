...
// var connectionServer = mongoose.createConnection('mongodb://projek20:projek20@198.211.106.64:27017/siagabanjir',{useNewUrlParser: true,useUnifiedTopology: true});

// var DpsMain = connection.model('DpsMain', dpsMainSchema,'main_dps');
var DpsMain = connection.model('DpsMain', dpsMainSchema,'main_dps');
// var DpsMainServer = connectionServer.model('DpsMainServer', dpsMainSchema,'main_dps_tes');
	...
	if(m==10||m==20||m==30|m==40 ||m==50||m==0){
		...
		st.forEach(function(st){
			...
			// DpsTemp.find({'site':st}).lean().exec(function(err,res){
				...
				// var dataToMongoServer = new DpsMainServer({ 
				// 	dt: new Date(moment().add(7,'hours').format()),
				// 	site: st,
				// 	tma: tma,
				// 	vair: vair,
				...
				// 	ch: [-1,1003]
				// });
				// var dataToMongo331 = new DpsMain({ 
				// 	dt: new Date(moment().add(7,'hours').format()),
				// 	site: 331,
				// 	tma: [-1,1003],
				// 	vair: [-1,1003],
				...
				console.log('')
					...
					// dataToMongoServer.save(function (err) {
					// 	if (err) return handleError(err);
					// 	console.log("Data disimpan")
					// });
					...
					// 	if (err) return handleError(err);
					// });
					// dataToMongo331.save(function (err) {
					// 	if (err) return handleError(err);
					// });
			})
...
setInterval(mean,60000) //looping 1 minute
